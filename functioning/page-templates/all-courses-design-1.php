...
<script>
    ...
    function fetchCourses(cpage = 1, type = 'general', category = '') {
        ...
        fetch(`${apiUrl}?cpage=${cpage}&per_page=${perPage}&type=${type}&category=${category}`, {
            ...
            .then(data => {
                //const totalCourses = data.total_courses || (data.total_pages * perPage);
                displayCourses(data.courses);
                setupPagination(data.total_pages, cpage, type, category);
                updateCourseShowingText(cpage, perPage, data.total_courses, data.courses.length); // Update the showing text
    ...
    function updateCourseShowingText(currentPage, perPage, totalCourses, currentCoursesOnPage) {
        const showingTextContainer = document.querySelector('.course-showing-part-left p');
        if (!showingTextContainer) return;

        const start = (currentPage - 1) * perPage + 1;
        const end = start + currentCoursesOnPage - 1;

        showingTextContainer.textContent = `Showing ${start}-${end} of total results`;
    }
