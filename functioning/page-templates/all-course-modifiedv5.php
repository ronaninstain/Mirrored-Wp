...
<script>
    ...
    const apiUrl = 'somesites.com/wp-json/custom/v1/connected-courses/';
    ...
    function fetchCourses(cpage = 1, type = 'general', category = '') {
        ...
        fetch(`${apiUrl}?cpage=${cpage}&per_page=${perPage}&type=${effectiveType}&category=${category}&course_ids=${encodeURIComponent(clientCourseIds.join(','))}`, {
            method: 'GET',
            headers: {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        function loadCategories() {
            fetch('somesites.com/wp-json/custom/v1/course-categories/', {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${clientID}:${secretKey}`,
                    'Content-Type': 'application/json'
